# src/harness/train.jl — FROZEN
#
# Wraps ParametricDFT.train_basis with the frozen configuration from spec
# §Evaluation Protocol: RiemannianAdam(lr=0.01), 500 steps per image, seed 42,
# batch 1, no validation split. The basis is trained JOINTLY on every fixture
# in `load_fixtures()` (multi-fixture eval, Addendum C).
#
# The `steps` kwarg controls steps-per-image and is only lowered for tests —
# production trials always use TRAIN_STEPS.

using ParametricDFT: train_basis, RiemannianAdam, MSELoss, AbstractSparseBasis

"""
    train_trial(::Type{B}; images=load_fixtures(), steps=TRAIN_STEPS, device=_default_device())
        -> (trained_basis::B, final_mse::Float64, wallclock_ms::Float64)

Train a basis of type `B` on the fixture set with the frozen config, then
evaluate mean MSE across the same fixtures. Returns the trained basis and
its averaged MSE — the canonical metric for accept/reject decisions.
"""
function train_trial(::Type{B};
                     images::AbstractVector = load_fixtures(),
                     steps::Int = TRAIN_STEPS,
                     device::Symbol = _default_device(),
                     extra_kwargs...) where {B <: AbstractSparseBasis}
    Random.seed!(SEED)
    t0 = time_ns()
    basis, _history = train_basis(
        B, collect(images);
        m = M_QUBITS, n = N_QUBITS,
        loss = MSELoss(TOPK),
        epochs = 1,
        steps_per_image = steps,
        validation_split = 0.0,
        shuffle = false,
        early_stopping_patience = typemax(Int),
        optimizer = RiemannianAdam(lr = LEARNING_RATE),
        batch_size = 1,
        device = device,
        extra_kwargs...,
    )
    wallclock_ms = (time_ns() - t0) / 1e6
    final_mse = evaluate_basis(basis; image = images)
    return basis, final_mse, wallclock_ms
end

function _default_device()
    # Auto-detect: if CUDA is loaded and functional, use :gpu. `src/AutoDFT.jl`
    # performs `using CUDA` at module load, so ParametricDFT.CUDAExt is
    # available when we reach this point. Spec says MSE across :cpu and :gpu
    # agrees to atol=1e-6, so device selection doesn't affect accept/reject
    # decisions — only wall-clock.
    return CUDA.functional() ? :gpu : :cpu
end
